...
// command.Err.
func NewError(err *command.Err) *Error {
	if err == nil {
		return &Error{}
	}
	return &Error{
...
// AsError returns the *command.Err representation of an *Error.
func (err *Error) AsError() *command.Err {
	return AsError(err)
}

// AsError returns a command.Err value that wraps an Error.
func AsError(err *Error) *command.Err {
	if err == nil {
		return command.NewError(0, nil)
	}
	details := slice.Map(err.Details, (*ErrorDetail).AsErrorDetail)
	return command.NewError(command.ErrorCode(err.GetCode()), errors.New(err.GetMessage()), command.WithErrorDetails(details...))
}
