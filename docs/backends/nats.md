...
### Workflows with Core and JetStream

[Workflows](/guide/workflows) work with both NATS Core and JetStream.

With Core, the workflow service can recover recently missed trigger events from the event store during startup replay. This replay is **opt-in**: set `workflow.Config.TriggerReplayWindow` (e.g. `24*time.Hour`) to enable it — the zero value disables startup replay. Enable it whenever the trigger bus doesn't provide durable delivery on its own.
...
For distributed workflow services, configure queue/load-balancing on the trigger bus so only one instance handles each trigger event. Do **not** reuse that same load-balanced bus configuration as the transport for `cmdbus.New(...)`.
...
This keeps workflow trigger delivery distributed while preserving the command bus requirement that command coordination must not run on a load-balanced event bus.
