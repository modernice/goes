...
func newJetStreamBus(enc codec.Encoding[any]) event.Bus[any] {
...
func newDurableJetStreamBus(enc codec.Encoding[any]) event.Bus[any] {
	return nats.NewEventBus(
		enc,
		nats.EatErrors(),
		nats.Use(nats.JetStream()),
...
func newQueueGroupJetStreamBus(enc codec.Encoding[any]) event.Bus[any] {
	return nats.NewEventBus(
		enc,
		nats.EatErrors(),
		nats.Use(nats.JetStream()),
...
func newDurableQueueGroupJetStreamBus(enc codec.Encoding[any]) event.Bus[any] {
	return nats.NewEventBus(
		enc,
		nats.EatErrors(),
		nats.Use(nats.JetStream()),
